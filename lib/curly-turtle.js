...
function recievedRequest(request, response) {
	...
	if ( ! new RegExp("^" + programInfo.baseurl).test(pathname) ) {
		...
		case "":
			renderAndRespond(request, response, { status: "redirect", template: "redirect.html", httpCode: 301, location: pathname + "/" });
			break;
		...
		default:
			fs.readFile("./assets/static" + pathname.substring(programInfo.baseurl.length), function(error, file) {
				if (error) {
