	...
	if ( ! new RegExp("^" + programInfo.baseurl).test(pathname) ) {
		...
		case "/settings":
			...
			if (Object.getOwnPropertyNames(userSettings).length > 0) {
				...
				renderAndRespond(request, response, { status: "settings saved", template: "settings.html", headers: { "Set-Cookie": cookieString } });
			} else {
...
function rationalizeRequest(request) {
	try {
		var settings = JSON.parse(decodeURIComponent(cookie.parse(request.headers.cookie).ctSettings));
	} catch (e) {}
	return {
		url: request.url,
