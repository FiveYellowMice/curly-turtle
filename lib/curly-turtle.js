	...
	if ( ! new RegExp("^" + programInfo.baseurl).test(pathname) ) {
		...
		case "/json":
			...
			function respondJSON(data) {
				data.file = JSON.stringify(Object.assign({ request: { url: request.url, headers: request.headers } }, data), null, "\t");
				if (data.headers) {
		...
		case "/settings":
			...
			if (Object.getOwnPropertyNames(userSettings).length > 0) {
				var cookie = "ctSettings=" + encodeURIComponent(JSON.stringify(userSettings)) + "; expires=" + new Date(new Date().getTime()+31536000000).toUTCString() + "; path=" + programInfo.baseurl + "/";
				renderAndRespond(request, response, { status: "settings saved", template: "settings.html", headers: { "Set-Cookie": cookie } });
			} else {
				renderAndRespond(request, response, { status: "settings", template: "settings.html" });
			}
			break;
