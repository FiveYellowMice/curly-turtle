	...
	jsdom.env(rawHTML, function(error, window) {
		if (error) {
			...
			try {
				...
				var language = window.document.getElementsByTagName("html")[0].lang;
				...
				for (var i = 0; i < listElements.length; i++) {
					try {
						...
						console.error(chalk.cyan(listElements[i].outerHTML));
					}
				}
				...
				if (resultList.length === 0) {
					...
					callback({ status: "success", template: "results.html", keywords: keywords, resultstats: resultStats, pagenumber: pageNumber, language: language, results: resultList });
				}
